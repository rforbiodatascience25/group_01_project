...
finaldata <- finaldata |> 
  mutate(birth_date = str_split_i(characteristics_ch1.2, ":", -1), keep=FALSE)  |> 
  select(procedure_date, birth_date) |> 
...
finaldata <- finaldata |> 
  mutate(birth_date = str_split_i(characteristics_ch1.2, ":", -1), keep=FALSE)  |> 
  select(submission_date, birth_date) |> 
  mutate(birth_date = reformat_pheno_birth_dates(birth_date)) |> 
  mutate(submission_date = as.Date.character(submission_date, "%b %d %Y")) |> 
  mutate(Age = round(as.numeric(difftime(submission_date, birth_date, units = "days")) / 365.25, 0)) |> 
   mutate(age_group = case_when(
    20 <= Age & Age < 30 ~ "(20,30]",
    30 <= Age & Age < 40 ~ "(30,40]",
    40 <= Age & Age < 50 ~ "(40,50]",
