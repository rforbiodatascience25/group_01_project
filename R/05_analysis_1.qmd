```{r}
sig_genes <- de_results_annot |>
  filter(FDR < 0.05, abs(log2FC) > 0.25) |>
  arrange(FDR)
```
...
sig_genes <- plot_df |>
  ...
  select(gene_id, GENE_SYMBOL, log2FC, p_value, FDR, sig_cat) |>
  arrange(FDR)
...
ggplot(heat_df,
  ...
  labs(x = "Patient", y = "Gene Symbol", fill = "Expression",
       title = "Top 40 differential genes across patients") +
  theme_minimal() +
