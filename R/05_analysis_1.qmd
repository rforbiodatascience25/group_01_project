...
finaldata |> 
  distinct(patient, inflammation_status, smoking_status) |> 
  ggplot(aes(x = smoking_status, fill = inflammation_status)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  coord_flip() +
  scale_x_discrete(
    labels = function(x) {
      paste0(x, " (", eth_counts$n[match(x, eth_counts$smoking_status)], ")")
    }
  ) +
  theme_minimal() +
...
plot_df <- de_results_annot |>
  ...
  mutate(
    ...
    sig_cat = factor(sig_cat,
                     levels = c("Up in inflamed", "Up in uninflamed", "NS"))
  )
