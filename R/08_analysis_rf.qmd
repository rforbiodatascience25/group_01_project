...
ggplot(plot_df,
       # Use the original sig_cat for the base layer
       aes(x = log2FC, y = neg_log10_p, colour = sig_cat)) +
  # 1. Base points (all genes, colored by sig_cat)
  geom_point(size = 1.3, alpha = 0.8) +
  
  # Highlighted points (overlaid with a distinct color/border)
  # Filter plot_df to only include highlighted genes
  geom_point(data = filter(plot_df, highlight_cat == "Most Contributing"),
             aes(x = log2FC, y = neg_log10_p),
             colour = "black", # border color
             size = 2,
             show.legend = FALSE) +
  # Re-plot the point itself in a new color
  geom_point(data = filter(plot_df, highlight_cat == "Most Contributing"),
             aes(x = log2FC, y = neg_log10_p),
             colour = "green", # highlight color
             size = 2,
             show.legend = FALSE) +
  ...
  geom_vline(xintercept = c(-lfc_cut, lfc_cut),
             linetype = "dashed", linewidth = 0.3) +
  # labels for strongest hits (using label_df as before)
  geom_text_repel(
    data = label_df,
  ...
  # nicer colours (for the sig_cat legend)
  scale_colour_manual(
    values = c(
      "Up in inflamed"   = "#d73027",
      "Up in uninflamed" = "#4575b4",
      "Non significant"  = "grey80"
      # Note: The highlighted genes are colored by the added geom_point layers,
      # so you don't need to add the highlight color here unless you want a separate legend entry.
    ),
    name = NULL
  ) +
  labs(
