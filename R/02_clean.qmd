...
phenodata <- phenodata |>
  #Convert all columns starting with "characteristics_ch1" to long format
  pivot_longer(
  ...
  # 4) Convert back to wide: each prefix becomes its own column
  pivot_wider(
    names_from = prefix,
    values_from = content
  )
...
```{r}
phenodata <- phenodata |> 
  select(geo_accession, patient, current_medication, birth_date, ethnicity, symptoms_onset_date, diagnosis_date, joint_problems, uc_flare_up, family_history, neutrophils, albumin, ibd_affected_relatives, smoking_status, smoking_start_date, smoking_stop_date, smoking_amount, other_illnesses, disease, anatomic_location, inflammation_status)
```
...
```{r}
finaldata <-  finaldata |> 
  pivot_longer(cols = 118:last_col(),
               names_to = "gene_id" ,
               values_to = "gene_expr" )
...
Now we filter the same low variance genes from featuredata
...
```{r}
write_csv(finaldata, file = "../data/finaldata.csv.gz")
